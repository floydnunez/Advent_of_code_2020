...
function castRay(map, x, y, dx, dy) {//THIS WAS GETPOS AND CHANGED RADICALLY
    const h = map.length;
    const w = map[0].length;
    for (let ii = 1; ii < h+w; ii++) {
        const xx = x + ii * dx;
        const yy = y + ii * dy;
        if (xx < 0 || yy < 0 || xx >= w || yy >= h) {
            return '.';
        }
        const elem = map[yy][xx];
        if (elem !== '.') {
            return elem;
        }
    }
...
function occupied(map, x, y) {
    let count = 0;
    const adjacent = [castRay(map, x, y, -1, -1), castRay(map, x, y, 0, -1), castRay(map, x, y, 1, -1),
        castRay(map, x, y, 1,0), castRay(map, x, y, 1, 1), castRay(map, x, y, 0, 1), castRay(map, x, y, -1, 1),
        castRay(map, x, y, -1, 0)];
    return adjacent;
}
...
function apply_rule(count, elem) {
    ...
    if (elem === '#' && count.o >= 5) {//HERE THE RULE CHANGED FROM PART A!
        return 'L';
    }
    return elem;
}
